...
- **Ownership:** each mailbox secret holds a `MailboxRecord { owner, tokens }`. A mailbox is usable only if the caller's `UserRecord` lists it *and* the record names the caller as owner. Message and event ids are looked up across the caller's mailboxes only, and ids are confined to Graph's token alphabet (`check_id`) so `..\` can never reshape a Graph path.
- **One browser per flow:** the Continue step (`/authorize/go`, `/connect/go`) sets a cookie that `/callback` requires; a Microsoft sign-in URL opened in another browser completes nothing. Mailbox secrets are named `mailbox-<sanitized address>-<hash8>` (legacy `mailbox-<sanitized address>` names are still read and move on the next save).
